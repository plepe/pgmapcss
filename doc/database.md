...
* In Overpass API, bounding box queries do not include ways and relations which enclose the bounding box, with the exception of [areas](http://wiki.openstreetmap.org/wiki/Overpass_API/Areas) which are queried additionally. Therefore large polygons might be missing in the output.
