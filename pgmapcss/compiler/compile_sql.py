...
def compile_condition_overpass(condition, statement, tag_type, stat, prefix, filter):
    ret = None
    negate = False
    key = tag_type[1]
    op = condition['op']
    ...
    # =
        ...
        ret = '__TYPE__[' + repr(key) + '=' + repr(condition['value']) + ']'
...
def compile_condition_sql(condition, statement, stat, prefix='current.', filter={}):
    ...
    if tag_type is None:
        ...
        ret.add(compile_condition_overpass(condition, statement, tag_type, stat, prefix, filter))
    else:
